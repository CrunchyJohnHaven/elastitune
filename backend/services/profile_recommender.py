...
class ProfileRecommender:
    def recommend(
        ...
        recommended.vectorWeight = best_profile.vectorWeight if recommended.useVector else 0.0
        recommended.lexicalWeight = best_profile.lexicalWeight if recommended.useVector else 1.0

        benchmark_boosts = {item["field"]: item["boost"] for item in best_profile.lexicalFields}
    ...
    def _pick_benchmark(
        ...
        fields = " ".join(field["field"].lower() for field in baseline_profile.lexicalFields)

        if summary.detectedDomain == "security" or any(token in index for token in ["siem", "alert", "threat", "security"]):
            return "Security SIEM"
        if any(token in index for token in ["movie", "tmdb", "film"]) or any(token in fields for token in ["overview", "cast", "director"]):
            return "TMDB Movies"
        if any(token in index for token in ["book", "library"]) or any(token in fields for token in ["authors", "categories", "published_year"]):
            return "Books Catalog"
        if any(token in index for token in ["workplace", "policy", "hr"]) or any(token in fields for token in ["summary", "content", "role_permissions"]):
            return "Workplace Docs"
        if any(token in index for token in ["product", "catalog", "store"]) or any(token in fields for token in ["brand", "price", "product_type"]):

...
def build_lexical_query(
    query_text: str, profile: SearchProfile, size: int = 50
) -> Dict:
    """Build ES query from search profile."""
    fields_with_boosts = [f"{f['field']}^{f['boost']}" for f in profile.lexicalFields]
...
def weighted_fuse(
    ...
    final = {
        doc_id: lex_weight * lex_norm.get(doc_id, 0.0)
        + vec_weight * vec_norm.get(doc_id, 0.0)
...
class Evaluator:
    ...
    async def evaluate(
        self, profile: SearchProfile, eval_set: List[EvalCase]
    ...
    async def evaluate_demo(
        ...
        title_boost = next(
            (
                f["boost"]
                for f in profile.lexicalFields
                if f["field"] in ("title", "name")
            ),
            1.0,
        )
